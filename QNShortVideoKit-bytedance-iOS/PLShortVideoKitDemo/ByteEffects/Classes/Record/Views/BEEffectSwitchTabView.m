...
- (CGRect)indicatorLineFrameForIndex:(NSInteger)index {
    ...
    CGFloat itemWidth = [self itemWidthForIndexPath:index];
    return CGRectMake(attributes.frame.origin.x + (attributes.frame.size.width - itemWidth) / 2, self.bounds.size.height - 2, itemWidth, 2);
}
...
- (void)updateIndicatorLineFrameWithProportion:(CGFloat)proportion
{
    ...
    if (proportion < 0) {
        ...
        indicatorWidth = currentLayoutAttribute.bounds.size.width - 30 + proportionDecimal * (nextLayoutAttribute.bounds.size.width - currentLayoutAttribute.bounds.size.width);
        indicatorOffsetX = currentLayoutAttribute.frame.origin.x + 15 + proportionDecimal * currentLayoutAttribute.bounds.size.width;
    }
